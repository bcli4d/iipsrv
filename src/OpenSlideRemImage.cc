...
void OpenSlideRemImage::openImage() throw (file_error) {
...
void OpenSlideRemImage::loadImageInfo(int x, int y) throw (file_error) {

#ifdef DEBUG_OSI
  logfile << "OpenSlideRemImage :: loadImageInfo()" << endl;
...
void OpenSlideRemImage::closeImage() {
#ifdef DEBUG_OSI
  Timer timer;
  timer.start();
#endif
 ...
 */
RawTilePtr OpenSlideRemImage::getTile(int seq, int ang, unsigned int iipres, int layers, unsigned int tile) throw (file_error) {
  ...
  if (tile >= ntlx * ntly) {
    ostringstream tile_no;
    tile_no << "OpenSlideRemImage :: Asked for non-existant tile: " << tile;
    throw file_error(tile_no.str());
  }
 ...
 */
RawTilePtr OpenSlideRemImage::getCachedTile(const size_t tilex, const size_t tiley, const uint32_t iipres) {
 ...
 * @param res  	iipsrv's resolution id.  openslide's level is inverted from this.
 */
RawTilePtr OpenSlideRemImage::getNativeTile(const size_t tilex, const size_t tiley, const uint32_t iipres) {
 ...
 */
RawTilePtr OpenSlideRemImage::halfsampleAndComposeTile(const size_t tilex, const size_t tiley, const uint32_t iipres) {
...
void OpenSlideRemImage::bgra2rgb(uint8_t* data, const size_t w, const size_t h) {
 ...
 */
void OpenSlideRemImage::halfsample_3(const uint8_t* in, const size_t in_w, const size_t in_h,
...
void OpenSlideRemImage::compose(const uint8_t *in, const size_t in_w, const size_t in_h,
